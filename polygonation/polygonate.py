...
class Polygonate:
    ...
    def __polygonation(self, pickedge):
        ...
        neighbors = [[si for si in neighbors if si != -1]
                     for neighbors in self._delaunay.neighbors]

        if pickedge.startswith('long'):
            def f(cands): return np.argmax([cand['length'] for cand in cands])
        elif pickedge.startswith('round'):
            def f(cands): return np.argmin([cand['error'] for cand in cands])
        else:
            def f(cands): return np.argmin([cand['angles_before'].min()
                                         for cand in cands])
        ...
        def melt(si1, si2, shape3): # remove shapes with indices si1 and si2. Add shape with vertices shape3.
            ...
            def new_si(si):
                ...
                return si-2
            neighbors = [[new_si(si) for si in neighbors]
